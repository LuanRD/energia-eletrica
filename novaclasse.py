...
class Graficos:
    ...
    """
    Função que plota um "boxplot".
    """
    def boxplot(self):
    
        # Plot do Gráfico
        fig, ax = plt.subplots(figsize=(20,10))
        ax = sns.boxplot(data=Selecao(f'{self.__fonte}', self.__dados).seleciona(), orient='h', palette='rocket')
    ...
    def lineplot(self):
        ...
        sns.lineplot(data=Selecao(f'{self.__fonte}', self.__dados).seleciona(), palette='rocket', color='darkred')
    ...
    def mixedplot(self):       
        ...
        sns.barplot(data = Selecao_Soma(f'{self.__fonte}', self.__soma).seleciona(), x=Selecao_Soma(f'{self.__fonte}', self.__soma).seleciona().index.year.astype('string'), y=self.__fonte, alpha=0.5, ax=ax1, palette='rocket')
        ax2 = ax1.twinx()
